...
var initializeSchema = /* @__PURE__ */ __name(async (db) => {
...
var normalizePlayers = /* @__PURE__ */ __name((players) => {
  if (!Array.isArray(players)) {
    throw new Error("players must be an array");
  }
  const normalized = players.map((player) => String(player || "").trim().toLowerCase()).filter((player) => player.length > 0).sort();
  if (normalized.length < 1 || normalized.length > 2) {
    throw new Error("players must contain 1 or 2 names");
  }
  return normalized;
}, "normalizePlayers");
var formatQueueItem = /* @__PURE__ */ __name((row) => ({
  id: row.id,
...
var getAutoFinishMinutes = /* @__PURE__ */ __name((c) => {
  const raw2 = c.env.AUTO_FINISH_MINUTES || "17";
  const parsed = parseInt(raw2, 10);
  return Number.isNaN(parsed) ? 17 : Math.max(1, parsed);
}, "getAutoFinishMinutes");
var getCabinet = /* @__PURE__ */ __name(async (db, id) => {
  const result = await db.prepare("SELECT * FROM cabinets WHERE id = ?").bind(id).first();
  return result || null;
}, "getCabinet");
var getQueueItem = /* @__PURE__ */ __name(async (db, id) => {
  const result = await db.prepare("SELECT * FROM queue_items WHERE id = ?").bind(id).first();
  return result || null;
}, "getQueueItem");

// src/utils/queueOps.js
var finishQueueItem = /* @__PURE__ */ __name(async (db, item) => {
  if (!item) return;
  const maxResult = await db.prepare("SELECT MAX(position) AS max_position FROM queue_items WHERE cabinet_id = ?").bind(item.cabinet_id).first();
  const maxPosition = maxResult?.max_position ?? 0;
  await db.prepare(
    "UPDATE queue_items SET position = ?, is_playing = 0, started_at = NULL WHERE id = ?"
  ).bind(maxPosition + 1, item.id).run();
  const next = await db.prepare("SELECT * FROM queue_items WHERE cabinet_id = ? AND id != ? ORDER BY position ASC LIMIT 1").bind(item.cabinet_id, item.id).first();
  if (next) {
    await db.prepare('UPDATE queue_items SET started_at = datetime("now"), is_playing = 1 WHERE id = ?').bind(next.id).run();
  }
}, "finishQueueItem");
var expireOldItems = /* @__PURE__ */ __name(async (db, autoFinishMinutes) => {
  const expired = await db.prepare('SELECT * FROM queue_items WHERE started_at IS NOT NULL AND started_at < datetime("now", ?)').bind(`-${autoFinishMinutes} minutes`).all();
  if (!expired.results?.length) return;
  for (const item of expired.results) {
    await finishQueueItem(db, item);
  }
}, "expireOldItems");
var loadCabinetsWithItems = /* @__PURE__ */ __name(async (db) => {
  ...
  const cabinets2 = (cabinetsResult.results || []).map((cabinet) => ({
    ...cabinet,
    queue_items: []
  }));
  const cabinetMap = new Map(cabinets2.map((cabinet) => [cabinet.id, cabinet]));
  ...
  return cabinets2;
}, "loadCabinetsWithItems");
var getCabinetQueuePayload = /* @__PURE__ */ __name(async (db, cabinetId) => {
  const itemsResult = await db.prepare("SELECT * FROM queue_items WHERE cabinet_id = ? ORDER BY position ASC").bind(cabinetId).all();
...
}, "getCabinetQueuePayload");

// src/routes/cabinetRoutes.js
var cabinets = new Hono2();
...
cabinets.post("/", async (c) => {
  ...
  await c.env.arcadeq.prepare('INSERT OR IGNORE INTO cabinets (name, created_at, updated_at) VALUES (?, datetime("now"), datetime("now"))').bind(name).run();
...
cabinets.patch("/:id/reorder", async (c) => {
  ...
  const existingRows = await c.env.arcadeq.prepare(`SELECT id FROM queue_items WHERE id IN (${placeholder}) AND cabinet_id = ?`).bind(...newOrder, id).all();
  ...
  const positionsResult = await c.env.arcadeq.prepare(`SELECT id, position FROM queue_items WHERE id IN (${placeholder}) ORDER BY position ASC`).bind(...newOrder).all();
  const positions = (positionsResult.results || []).map((row) => row.position).sort((a, b) => a - b);
  for (let index = 0; index < newOrder.length; index += 1) {
    await c.env.arcadeq.prepare("UPDATE queue_items SET position = ? WHERE id = ?").bind(positions[index], newOrder[index]).run();
  }
  return c.json({ message: "Reordered" });
});
var cabinetRoutes_default = cabinets;
...
queue.post("/", async (c) => {
  ...
  const hash = Array.from(new Uint8Array(requestHash)).map((b) => b.toString(16).padStart(2, "0")).join("");
  const existing = await c.env.arcadeq.prepare("SELECT * FROM queue_items WHERE request_hash = ?").bind(hash).first();
  if (existing) {
    return c.json(formatQueueItem(existing), { status: 200 });
  }
  const maxResult = await c.env.arcadeq.prepare("SELECT MAX(position) AS max_position FROM queue_items WHERE cabinet_id = ?").bind(cabinetId).first();
  const position = (maxResult?.max_position ?? 0) + 1;
  const existingCount = await c.env.arcadeq.prepare("SELECT COUNT(*) AS cnt FROM queue_items WHERE cabinet_id = ?").bind(cabinetId).first();
  const isFirst = (existingCount?.cnt ?? 0) === 0;
  await c.env.arcadeq.prepare(
    ...
    ownerId,
    isFirst ? 1 : 0,
    isFirst ? (/* @__PURE__ */ new Date()).toISOString().replace("T", " ").slice(0, 19) : null
  ).run();
  const inserted = await c.env.arcadeq.prepare("SELECT * FROM queue_items WHERE request_hash = ?").bind(hash).first();
...
queue.get("/:cabinetId/time-to-finish", async (c) => {
  ...
  const row = await c.env.arcadeq.prepare("SELECT * FROM queue_items WHERE cabinet_id = ? ORDER BY position ASC LIMIT 1").bind(cabinetId).first();
...
queue.post("/:id/cycle", async (c) => {
  ...
  try {
    ...
    await db.batch([
      db.prepare('UPDATE queue_items SET is_playing = 0, started_at = NULL, position = ?, updated_at = datetime("now") WHERE id = ?').bind(targetBackPosition, id),
      db.prepare("UPDATE queue_items SET position = position - 1 WHERE cabinet_id = ? AND position > ? AND id != ?").bind(item.cabinet_id, item.position, id)
    ]);
    const next = await db.prepare("SELECT * FROM queue_items WHERE cabinet_id = ? ORDER BY position ASC LIMIT 1").bind(item.cabinet_id).first();
    if (next) {
      await db.prepare('UPDATE queue_items SET started_at = datetime("now"), is_playing = 1 WHERE id = ?').bind(next.id).run();
    }
...
queue.post("/:id/move", async (c) => {
  ...
  const maxResult = await c.env.arcadeq.prepare("SELECT MAX(position) AS max_position FROM queue_items WHERE cabinet_id = ?").bind(targetCabinetId).first();
  const newPosition = (maxResult?.max_position ?? 0) + 1;
  await c.env.arcadeq.prepare('UPDATE queue_items SET cabinet_id = ?, position = ?, updated_at = datetime("now") WHERE id = ?').bind(targetCabinetId, newPosition, id).run();
...
queue.patch("/:id", async (c) => {
  ...
  const collision = await c.env.arcadeq.prepare("SELECT id FROM queue_items WHERE request_hash = ? AND id != ?").bind(newHash, id).first();
  if (collision) {
    return c.json({ message: "Duplicate queue entry detected" }, { status: 422 });
  }
  await c.env.arcadeq.prepare('UPDATE queue_items SET players = ?, request_hash = ?, updated_at = datetime("now") WHERE id = ?').bind(JSON.stringify(players), newHash, id).run();
  const updated = await getQueueItem(c.env.arcadeq, id);
  return c.json(formatQueueItem(updated));
});
var queueRoutes_default = queue;
...
app.route("/api/cabinets", cabinetRoutes_default);
app.route("/api/queue", queueRoutes_default);
var src_default = {
  fetch: app.fetch,
...
// .wrangler/tmp/bundle-vrCEuc/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-vrCEuc/middleware-loader.entry.ts
